...
class WorldModuleClass {
  ...
  async load(): Promise<void> {
    ...
    if (wasm.default?.then) {
      (wasm as { default: Promise<typeof WorldWasm> }).default.then(
        data => this._module = data
      ).then(
        () => console.log("Modules Loaded ES6", this._module)
      )
    } else {
  ...
  public createWorld(
    ...
    return new World(
      wasmWorld, game, chunkReader, data
    );
  }

  public createChunk(chunkPos: Vector2D): Chunk {
    const wasmChunk = this.module.Chunk.make_wasm(chunkPos.get(0), chunkPos.get(1));
  ...
  public createChunkFromSerialized(data: ISerializedChunk): Chunk {
    ...
    const chunkPos = new Vector2D([data.chunkPos.x, data.chunkPos.y]);
    return new Chunk(wasmChunk, chunkPos);
  }
