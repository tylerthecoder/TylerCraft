...
impl Entity {
    ...
    pub fn set<T: Component + 'static>(&mut self, component: T) {
        self.components.retain(|c| c.as_any().type_id() != TypeId::of::<T>());
        self.components.push(Box::new(component));
    }

    pub fn has<T: Component + 'static>(&self) -> bool {
        self.components.iter().any(|c| c.type_id() == TypeId::of::<T>())
    }
...
impl<'a> EntityQueryResults<'a> {
    ...
    // pub fn get_entity_mut(&mut self, id: EntityId) -> Option<&'a mut Entity> {
    //     self.entities.iter_mut().find_map(|entity| {
    //         if entity.id == id {
    //             Some(entity)
    //         } else {
    //             None
    ...
    // Add methods to work with the filtered entities if needed
...
impl EntityHolder {
    ...
    pub fn query(&mut self, filter: &EntityQuery) -> EntityQueryResults {
        let filtered_entities = self.entities
            .iter_mut()
            .filter(|entity| filter.type_ids.iter().all(|&type_id|
                entity.has_typeid(type_id)
            ))
            .collect();
